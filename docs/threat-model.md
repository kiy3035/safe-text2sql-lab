...
운영 요청의 정답을 자동 보증하지 않는다. 현재 Ollama 모델이 없어 자연어 50건 정확도는
`PENDING`이며 수치를 추정할 수 없다.
...
Ollama 모델 파일의 출처·무결성·라이선스·프롬프트 데이터 보존 정책은 아직 검증하지 않았다.
base URL을 loopback으로 제한해 외부 전송을 막지만, 로컬 호스트의 다른 악성 프로세스나 이미
변조된 모델을 방어하지 않는다. 이 실행에서는 Ollama와 모델을 설치하지 않았다.
...
- WireMock 재시도 측정은 HTTP 상태와 지연 정책을 재현한다. 실제 Ollama의 추론 시간과 장애 특성을
  측정한 값은 아니다.
- Golden SQL 50건이 실행된 사실과 로컬 LLM이 50건을 맞힌다는 주장은 다르다. 후자는 아직
  측정하지 않았으므로 `PENDING`이다.
