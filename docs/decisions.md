...
- 아키텍처, 위협 모델, 실험 보고서, 블로그 초안을 분리한다. 구현 흐름, 방어 가능/불가능 범위,
  원본 수치, 서술형 글의 역할이 달라 한 파일에 섞으면 검토와 갱신이 어려워지기 때문이다.
- 블로그의 측정 수치는 5단계에 보존한 CSV/JSON만 사용한다. 자연어 정확도처럼 값이 null인 항목은
  문장으로 추정하지 않고 `PENDING`을 유지한다.
- `scripts/verify_documented_results.ps1`로 질문·Golden·보안 fixture·plan·retry 원본 개수와
  문서의 인덱스/재시도 수치, 측정 SHA, 전체 테스트 XML을 다시 대조한다. 문서 수정도 수치 회귀를
  만들 수 있으므로 자동 테스트 다음에 이 검증을 실행한다.
- Mermaid는 요청·신뢰 경계와 DB 계정 분리에만 사용한다. 세부 정책은 표와 코드 위치를 함께 적어
  다이어그램만으로 보안 보장을 오해하지 않게 한다.
- README에는 Ollama/model을 실제로 실행한 것처럼 쓰지 않는다. 사용자가 별도로 준비한 경우의
...
- 데모 화면은 React나 외부 CDN을 추가하지 않고 Spring Boot의 정적 HTML/CSS/JavaScript로 만든다.
  질문 하나와 결과 표만 필요한 포트폴리오 데모이므로 별도 프런트엔드 빌드·배포 계층을 두지 않는다.
- UI는 기존 `POST /api/v1/text2sql/query`의 클라이언트일 뿐 보안 경계가 아니다. SQL 검증과 DB
  권한은 서버가 계속 강제하고, 화면은 서버 결과를 `textContent`로만 렌더링해 HTML로 해석하지 않는다.
- 사용자의 반복 환경 변수 입력을 없애기 위해 시작 스크립트가 최초 한 번 무작위 DB 비밀번호를
  만들고 Git에서 제외된 `.env`에 저장한다. 공개 저장소에 고정 비밀번호를 넣는 편의는 선택하지 않는다.
- 실행 스크립트는 충돌한 DB·HTTP 포트를 자동으로 빈 포트로 바꾸고 실제 주소를 출력한다. Docker와
  Ollama 설치·모델 다운로드는 시스템 변경이므로 자동화하지 않고 준비 여부만 명확히 검사한다.
- 8GB 환경에서는 `bootRun`이 Gradle JVM과 애플리케이션 JVM을 함께 유지해 Docker·Ollama와 자원
  경합을 일으켰다. `bootJar` 완료 후 Gradle을 종료하고 `java -jar` JVM 하나만 실행하도록 정했다.
- DB 통합 테스트는 외부 셸의 `SQL_GENERATOR_PROVIDER` 값과 무관하게 `disabled` provider를 명시한다.
  실제 데모를 실행한 PowerShell 상태가 “Ollama 없이 시작” 테스트의 의미를 바꾸지 않게 하기 위해서다.
