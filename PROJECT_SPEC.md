...
- Spring Boot 정적 리소스의 `/`에서 자연어 질문 하나를 입력하고 결과 표를 확인할 수 있어야 한다.
- 화면은 기존 `POST /api/v1/text2sql/query`만 호출하며 별도의 SQL 실행 경로나 검증 우회 기능을 만들지 않는다.
- 컬럼명과 DB 결과를 포함한 서버 응답은 신뢰할 수 없는 표시 데이터로 취급하고 HTML 문자열이 아닌 텍스트 노드로 렌더링한다.
- 생성 SQL 원문, PostgreSQL 오류 원문, DB 비밀번호는 화면에 노출하지 않는다.
- 외부 CDN이나 프런트엔드 빌드 도구 없이 로컬에서 재현할 수 있어야 한다.
- Windows 시작 스크립트는 무작위 로컬 비밀번호를 Git 제외 `.env`에 보존해 반복 입력을 없애되, 프로그램·모델 설치를 자동 수행하지 않는다.
